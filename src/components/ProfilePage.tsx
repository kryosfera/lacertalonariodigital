...
import {
  ...
  Image as ImageIcon, Signature, PenTool, MapPin, ChevronsUpDown, LogOut, LifeBuoy, MessagesSquare,
} from "lucide-react";
...
import { MyTicketsSheet } from "@/components/tickets/MyTicketsSheet";
...
export const ProfilePage = () => {
  ...
  return (
    <>
      <div className="screen-wrapper">
        ...
        <div className="screen-body space-y-2 max-w-2xl mx-auto w-full">
          ...
          <SectionCard
            ...
            title="Soporte"
            description="Reporta incidencias y consulta respuestas del equipo"
          >
            <div className="flex flex-wrap gap-2">
              <Button size="sm" className="rounded-full h-9 text-xs" onClick={() => setTicketDialogOpen(true)}>
                <LifeBuoy className="w-3.5 h-3.5 mr-1.5" />
                Reportar incidencia
              </Button>
              <Button variant="outline" size="sm" className="rounded-full h-9 text-xs" onClick={() => setTicketsSheetOpen(true)}>
          ...
          >
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input id="clinic_name" placeholder="Clínica Dental..." value={formData.clinic_name} onChange={(e) => handleInputChange('clinic_name', e.target.value)} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="clinic_address" className="text-xs">Dirección</Label>
                <Textarea id="clinic_address" placeholder="Calle, número..." value={formData.clinic_address} onChange={(e) => handleInputChange('clinic_address', e.target.value)} rows={2} />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input id="locality" placeholder="Madrid, Sevilla..." value={formData.locality} onChange={(e) => handleInputChange('locality', e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Popover open={provinceOpen} onOpenChange={setProvinceOpen}>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className={cn("w-full justify-between font-normal h-10", !formData.province && "text-muted-foreground")}
                      >
                          ...
                          <CommandGroup>
                            {SPAIN_PROVINCES.map((p) => (
                              <CommandItem
                                ...
                                value={p}
                                onSelect={(val) => {
                                  const match = SPAIN_PROVINCES.find((x) => x.toLowerCase() === val.toLowerCase());
                                  handleInputChange('province', match || p);
                                  setProvinceOpen(false);
                                }}
                              >
                                <Check className={cn("mr-2 h-4 w-4", formData.province === p ? "opacity-100" : "opacity-0")} />
                                {p}
                              </CommandItem>
                            ))}
                          </CommandGroup>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input id="professional_name" placeholder="Dr. Juan García" value={formData.professional_name} onChange={(e) => handleInputChange('professional_name', e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="registration_number" className="text-xs">Nº Colegiado</Label>
                  <Input id="registration_number" placeholder="12345" value={formData.registration_number} onChange={(e) => handleInputChange('registration_number', e.target.value)} />
                </div>
              </div>
            </div>
          </SectionCard>

          <SectionCard icon={<ImageIcon className="w-4 h-4" />} title="Logotipo" description="Se mostrará en el encabezado de tus recetas">
            ...
            <div className="flex items-center gap-4">
              ...
              <Button variant="outline" size="sm" className="rounded-full h-9 text-xs" onClick={() => logoInputRef.current?.click()} disabled={uploadImage.isPending}>
                {uploadImage.isPending ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Upload className="w-3.5 h-3.5 mr-1.5" />}
                Subir logo
              </Button>
            </div>
          </SectionCard>

          <SectionCard icon={<Signature className="w-4 h-4" />} title="Firma digital" description="Se añadirá automáticamente a tus recetas">
            <div className="space-y-4">
              {signaturePreview && (
              ...
              <Tabs defaultValue="draw" className="w-full">
                <TabsList className="grid w-full grid-cols-2 h-9">
                  <TabsTrigger value="draw" className="text-xs"><PenTool className="w-3.5 h-3.5 mr-1" />Dibujar</TabsTrigger>
                  <TabsTrigger value="upload" className="text-xs"><Upload className="w-3.5 h-3.5 mr-1" />Subir archivo</TabsTrigger>
                </TabsList>
                <TabsContent value="draw" className="mt-3">
                  <SignaturePad onSave={handleSignaturePadSave} />
                  <p className="text-[11px] text-muted-foreground mt-2">Usa el dedo, un stylus o el ratón para dibujar tu firma</p>
                </TabsContent>
                <TabsContent value="upload" className="mt-3">
                  <input ref={signatureInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => handleFileChange(e, 'signature')} />
                  <Button variant="outline" size="sm" className="rounded-full h-9 text-xs" onClick={() => signatureInputRef.current?.click()} disabled={uploadImage.isPending}>
                    {uploadImage.isPending ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Upload className="w-3.5 h-3.5 mr-1.5" />}
                    Subir imagen de firma
                  </Button>
                  <p className="text-[11px] text-muted-foreground mt-2">Tip: Firma en papel blanco, hazle una foto y súbela aquí</p>
                </TabsContent>
              </Tabs>
            </div>
          </SectionCard>
          ...
          {hasChanges && (
            <div className="sticky bottom-20 md:bottom-4 flex justify-end z-10">
              <Button onClick={handleSave} disabled={upsertProfile.isPending} className="shadow-lg rounded-full">
                {upsertProfile.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Check className="w-4 h-4 mr-2" />}
                Guardar cambios
              </Button>
            </div>
          )}
      ...
      <MyTicketsSheet open={ticketsSheetOpen} onOpenChange={setTicketsSheetOpen} />
    </>
  );
};
